...
layui.define(["table", "form"],
    function(e) {
        ...
        i.render({
            ...
            i.on("tool(app-pay-qrcode-list)",
                function(e) {
                    var s = e;
                    if ("del" === e.event) layer.confirm("确定删除此二维码？",
                        function(d) {
                            t.ajax({
                                url: 'delQrcode?id='+ e.data.id,
                                method:'POST',
                                success:function (res) {
                                    if (res.code == 1){
                                        e.del()
                        ...
                        layer.open({
                            type: 2,
                            title: "编辑二维码",
                            content: "editQrcode?id=" + e.data.id,
                            maxmin: !0,
                            area: ['80%','60%'],
                            btn: ["确定", "取消"],
                            yes: function(e, f) {
                                var r = window["layui-layer-iframe" + e],
                                    l = "app-pay-qrcode-submit",
                                    o = f.find("iframe").contents().find("#" + l);
                                r.layui.form.on("submit(" + l + ")",
                                    function(r) {
                                        var l = r.field;
                                        //提交修改
                                        t.post("editQrcode",l,function (res) {
                                            if (res.code == 1){
                                                //更新数据表
                                                s.update({
                                                    name: l.name,
